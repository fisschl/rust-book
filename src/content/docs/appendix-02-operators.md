...
本附录包含 Rust 语法的词汇表，包括运算符以及单独出现或在路径、泛型、trait 约束、宏、属性、注释、元组和括号上下文中出现的其他符号。

## 运算符

表 B-1 包含 Rust 中的运算符、运算符在上下文中的示例、简短说明以及该运算符是否可重载。如果运算符可重载，列出了用于重载该运算符的相关 trait。
...
以下表格包含所有不作为运算符的符号；也就是说，它们的行为不像函数或方法调用。
...
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `'ident`                                                               | 命名生命周期或循环标签                                           |
| 紧跟 `u8`, `i32`, `f64`, `usize` 等的数字  | 特定类型的数字字面量                                       |
| `"..."`                                                                | 字符串字面量                                                         |
| `r"..."`, `r#"..."#`, `r##"..."##`, 等等                          | 原始字符串字面量；不处理转义字符                    |
| `b"..."`                                                               | 字节字符串字面量；构造字节数组而不是字符串  |
| `br"..."`, `br#"..."#`, `br##"..."##`, 等等                      | 原始字节字符串字面量；原始字符串和字节字符串字面量的组合    |
| `'...'`                                                                | 字符字面量                                                      |
| `b'...'`                                                               | ASCII 字节字面量                                                     |
...
表 B-3 显示了在通过模块层次结构到项目的路径上下文中出现的符号。
...
| `<type>::...`                           | 不能直接命名的类型的关联项（例如，`<&T>::...`, `<[T]>::...`, 等等） |
| `trait::method(...)`                    | 通过命名定义它的 trait 来消除方法调用的歧义                                             |
| `type::method(...)`                     | 通过命名为其定义的类型来消除方法调用的歧义                                       |
...
| `path::<...>`, `method::<...>` | 在表达式中指定泛型类型、函数或方法的参数；通常称为 _turbofish_（例如，`"42".parse::<i32>()`） |
| `fn ident<...> ...`            | 定义泛型函数                                                                                                                             |
| `struct ident<...> ...`        | 定义泛型结构体                                                                                                                            |
| `enum ident<...> ...`          | 定义泛型枚举                                                                                                                          |
| `impl<...> ...`                | 定义泛型实现                                                                                                                       |
| `for<...> type`                | 高阶生命周期边界                                                                                                                       |
| `type<ident=type>`             | 一个泛型类型，其中一个或多个关联类型具有特定赋值（例如，`Iterator<Item=T>`）                                       |

表 B-5 显示了在 trait 约束约束泛型类型参数的上下文中出现的符号。
...
表 B-6 显示了在调用或定义宏以及为项目指定属性的上下文中出现的符号。
